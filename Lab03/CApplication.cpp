...
void Application::Run() {
	event.setEventName("tests"); // 현재 버젼에서 이벤트는 tests 하나
	while (1) {
...
int Application::AddContents() {
	...
	if (m_List.Add(item) == 1) {
		cout << item.GetEventName()<<endl;
		cout << event.getEventName()<<endl;
		if (event.getEventName() == item.GetEventName()) {
			event.AddMC(item); //파일의 이벤트명이 현재App에서 가지고 있는 이벤트명과 같다면 이벤트리스트에 추가
		}
		return 1;
	}
...
int Application::DeleteContents() {
	...
	if (m_List.Delete(item)) {
		m_appFC.DeleteMContents(item);//FC 리스트에서도 삭제
		if (event.getEventName() == item.GetEventName()) {
			event.Delete(item);//파일의 이벤트명이 현재App에서 가지고 있는 이벤트명과 같다면 이벤트리스트에 추가
		}
...
// 이벤트 정보 출력
void Application::DisplayEvents() {
	cout << "\t=====이벤트 리스트=====" << endl;
