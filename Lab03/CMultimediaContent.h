...
public:
	...
	*/
	...
	/**
	*	두 MultimediaContent를 비교
	*	@brief	두 MultimediaContent을 이름으로 비교
	*	@pre	두 MultimediaContent이 초기화 됨
	*	@param	data	비교대상이 되는 MultimediaContent객체
	*	@return	return EQUAL if this.name == data.name,
	*			return LESS if this.id < data.id
	*			return GREATER if this.id > data.id
	*/

	bool operator< (const MultimediaContent &mc) const{
