...
public:
	...
	*/
	...
	/**
	*	@brief	조건을 받아, 해당조건을 만족하게 리스트를 정렬
	*	@pre	리스트가 초기화 됨
	*	@post	리스트가 정렬됨
	*	@param	bool (*Sfunc)(const T&, const T&)	리스트의 두개의 나란한 원소를 입력으로 받고 반환형이 bool인 함수 포인터
	*/
	void Sort(bool (*Sfunc)(const T&, const T&));
...
// 조건을 입력받아, 해당조건을 만족하게 리스트를 정렬
template <class T >
void List<T>::Sort(bool (*Sfunc)(const T&, const T&)) {
	for (int i = 0; i < m_Length - 1; i++) {
		for (int j = 0; j < m_Length - i - 1; j++) {
			if (!Sfunc(m_Array[j], m_Array[j + 1])) { // 버블정렬
				T tmp = m_Array[j];
				m_Array[j] = m_Array[j + 1];
				m_Array[j + 1] = tmp;
			}
